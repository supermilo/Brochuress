<div id="warp" class="theme-opts wrap">

	<h2>Theme Options</h2>
	<p><?php $data = $xml->first('description'); echo $data->text(); ?></p>
	...
	<div class="sidebar" style="width:280px;margin-right:20px;float:right;">
		<div class="box postbox">
			...
			<div class="content">
				<ul class="list">
					<li>Name: <?php $data = $xml->first('name'); echo $data->text(); ?></li>
					<li>Version: <?php $data = $xml->first('version'); echo $data->text(); ?></li>
					<li>Warp Framework Version: <?php $data = $warp_xml->first('version'); echo $data->text(); ?></li>
					<li>Created: <?php $data = $xml->first('creationDate'); echo $data->text(); ?></li>
		...
		<div class="box postbox">
			...
			<div class="content">
				<ul class="list">
					...
					<li>Need help? Just head over to the <a href="http://www.yootheme.com/support/documentation">documentation</a>.</li>
				...
				<?php
					foreach ($settings as $node) {
				        echo '<div class="option">';
   				        echo '<h4>'.$node->attr('label').'</h4>';
						echo '<div class="value">'.$this->warp->control->render($node->attr('type'), $this->warp->system->prefix.$node->attr('name'), $this->warp->config->get($node->attr('name')), $node).'</div>';
   				        echo '<span class="description">'.$node->attr('description').'</span>';
				        echo '</div>';
			        }
				?>
				</div>
