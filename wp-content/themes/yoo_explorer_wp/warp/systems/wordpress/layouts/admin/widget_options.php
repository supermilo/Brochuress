<div id="warp" class="widget-opts wrap">
	    		...
	    		if ($position_settings[$position]['configurable']) {
	                ...
	                foreach ($widgets as $widget) {
						...
						foreach ($module_settings as $node) {
							...
							$html[] = '<div class="value">'.$this->warp->control->render($node->attr('type'), 'warp_widget_options['.$widget->id.']['.$name.']', $value, $node, compact('widget')).'</div>';
					        $html[] = '</div>';
						}
