...
class UT_Admin_home {
    ...
    function dashboard_notification() {
        ...
        if( isset( $_GET['page'] ) && in_array( $_GET['page'], $allow_panels ) ) {
            ...
            if( !empty( $theme_updates_available->response['brooklyn'] ) ) {

                // Theme Notification
                $message = sprintf( esc_html__( 'New theme update available. %s. %s', 'unite-admin' ), '<span>Brooklyn ' . $theme_updates_available->response['brooklyn']['new_version'] . '</span>', sprintf( '<a href="%s">' . esc_html__( 'Update Now','unite-admin' ) . '</a>', admin_url('themes.php') ) );
                ...
                // Plugin Notification
                $message = sprintf( esc_html__( 'New updates are available for the following plugins: %s. %s', 'unite-admin' ), $plugin_updates, sprintf( '<a href="%s">' . esc_html__( 'Update Now','unite-admin' ) . '</a>', admin_url('plugins.php') ) );
