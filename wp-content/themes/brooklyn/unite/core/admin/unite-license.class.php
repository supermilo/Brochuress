...
class UT_Licensing {
                ...
                <div id="ut-ui-admin-header">
                    ...
                    <div class="grid-90 medium-grid-85 tablet-grid-80 mobile-grid-100 grid-parent">
                        ...
                        <div class="ut-admin-header-title">
                            ...
                            <h1><?php esc_html_e( 'Brooklyn License', 'unite-admin' ); ?></h1>
                ...
                <div id="ut-dashboard-content">
                    ...
                    <div class="grid-70 prefix-15 medium-grid-100 tablet-grid-100 mobile-grid-100">
                        ...
                        <div class="ut-dashboard-hero">
                            ...
                            <div class="hide-on-mobile"><?php esc_html_e( 'After buying and installing Brooklyn from ThemeForest, registering is an important step to ensure you will receive valuable theme updates that include new features and keep your website secure.', 'unite-admin' ); ?></div>
                    ...
                    <div class="ut-dashboard-widgets clearfix">
                        ...
                        <div id="ut-dashboard-widget-code-info" class="ut-dashboard-widget clearfix" style="text-align: center">
                            ...
                            <div class="ut-dashboard-hover">
                                ...
                                <div class="ut-rotate-label-hover">If you want to use Brooklyn on another domain, please purchase another license.</div>

                            </div>

                            <form method="post" action="https://helpdesk.unitedthemes.com/index.php/Thread/320-Purchase-Key/" style="margin-top: 10px;">
                                <button type="submit" formtarget="_blank" class="ut-ui-button">Find my Code</button>
                            </form>
                        ...
                        <div id="ut-dashboard-widget-license-info" class="ut-dashboard-widget ut-dashboard-widget-large clearfix">
                            ...
                            <h3 class="ut-dashboard-title" style="margin-bottom: 10px;"><?php _e('Register Purchase Code', 'unite') ?></h3>
                            ...
                            <form method="post" action="options.php">
                                ...
                                <p>
	                                ...
	                                <?php if( $this->get_license_info() == 'unregistered' ) : ?>
                                        ...
                                        <input id="ut-register-license" type="submit" class="ut-ui-button" value="<?php _e('Register Brooklyn', 'unite') ?>" disabled="" />

	                                <?php endif; ?>

                                    <?php if( $this->get_license_info() != 'unregistered' ) : ?>

                                        <input id="ut-deregister-license" type="submit" class="ut-ui-button ut-ui-button-health" value="<?php _e('Deregister Domain', 'unite') ?>" />

                                    <?php endif; ?>

                                </p>

                                <?php if( $this->get_license_info() == 'registered'|| $this->get_license_info() == 'registered_supported' ) : ?>
                                    ...
                                    <ul class="ut-license-info">
                                        <li><span><?php esc_html_e( 'Registered Domain', 'unitedthemes' ); ?></span> : <?php echo esc_url( $this->get_site_url_plain() ); ?></li><li><span><?php esc_html_e( 'Registered Server IP', 'unitedthemes' ); ?></span> : <?php echo $host; ?></li><li><span><?php esc_html_e( 'Registered Email', 'unitedthemes' ); ?></span> : <?php echo get_bloginfo('admin_email'); ?></li>
                                    </ul>
    ...
    public function remote_license_info( $purchase_code = '', $return = false, $action = '' ) {
	    ...
	    if( $ajax_request || false === ( $request = get_transient( 'ut-license-information' ) ) || empty( get_transient( 'ut-license-information' ) ) ) {
		    ...
		    if( is_wp_error( $request ) ) {
			    ...
			    $request = '{"result" : "connection_issue", "reason" : "Connection Failed! Please try again in 15 Minutes."}';

			    // set a 15 Minutes Transient to prevent API Hammering
			    set_transient( 'ut-license-information', (object) array( 'result' => 'connection_issue', 'user' => '', 'supported' => '' ), 15 * MINUTE_IN_SECONDS  );
	...
	public function set_server_status() {
		...
		if ( false === ( $request = get_transient( 'ut-server-status' ) ) ) {
			...
			if( is_wp_error( $request ) ) {
				...
				set_transient( 'ut-server-status', (object) array( 'server_status', 'offline' ), HOUR_IN_SECONDS );
	...
	public function set_license_benefits() {
		...
		if ( false === ( $request = get_transient( 'ut-license-benefits' ) ) ) {
			...
			if( is_wp_error( $request ) ) {
				...
				set_transient( 'ut-license-benefits', (object) array( 'server_status', 'offline' ), HOUR_IN_SECONDS );
	...
	function theme_info( $prepared_themes ) {
	    ...
	    if( isset( $prepared_themes['brooklyn'] ) && $this->get_license_info() == 'unregistered' ) {
		    ...
		    $prepared_themes['brooklyn']['description'] = $prepared_themes['brooklyn']['description'] . '<br /><div class="ut-missing-license">' . sprintf( esc_html__( 'You need a valid license for automatic updates!  %s or register your license here %s', 'unite-admin' ), '<a href="' . $this->theme_url . '" target="_blank">'. esc_html__('Purchase Brooklyn', 'unite-admin') . '</a>', '<a href="' . get_admin_url() . 'admin.php?page=unite-manage-license">'. esc_html__('Register Brooklyn', 'unite-admin') . '</a>' ) . '</div>';
	...
	public function enqueue_import_css() {
		...
		$localized_array = array(
			'missing_license'  => sprintf( esc_html__( 'You need a valid license for automatic updates!  %s or register your license here %s', 'unite-admin' ), '<a href="' . $this->theme_url . '" target="_blank">'. esc_html__('Purchase Brooklyn', 'unite-admin') . '</a>', '<a href="' . get_admin_url() . 'admin.php?page=unite-manage-license">'. esc_html__('Register Brooklyn', 'unite-admin') . '</a>' ),
		);
	...
	public function enqueue_import_js() {
		...
		$localized_array = array(
			'message'  => esc_html__( 'Are you sure?', 'unite-admin' ),
		);
