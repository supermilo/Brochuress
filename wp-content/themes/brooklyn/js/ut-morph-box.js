...
`;
    ...
    $(window).load(function(){
        ...
        if( site_settings !== "undefined" && site_settings.lg_type === 'morphbox' ) {
            ...
            const UT_Morph_Box_APP = {
                ...
                create_morph_app: function() {
                    ...
                    $('a[data-exthumbimage]').not('.ut-morphbox-ready').each(function() {
                        ...
                        let small_title_found = false;

                        let $morph_small_title = $('<h3></h3>');
                        let $morph_title = $('<p></p>');

                        // Data Caption
                        let $morph_title_wrap = $('<div class="ut-morph-box-item-wrap"></div>');

                        if( $this.data('title') && $this.data('title') !== '' ) {

                            $morph_small_title.text( $this.data('title') );

                            $morph_title_wrap.append( $morph_small_title );
                            $morph_item.append( $morph_title_wrap );

                            small_title_found = true;

                        }

                        if( !title_found && $this.data('caption') && $this.data('caption') !== '' ) {

                            $morph_title.text( $this.data('caption') );
                        ...
                        if( !title_found && $this.data('sub-html') && $this.data('sub-html') !== '' ) {

                            $morph_title.text( $( $this.data('sub-html') ).text() );
                        ...
                        if( !title_found && $( imageSet.small.element ).attr('alt') !== undefined && $( imageSet.small.element ).attr('alt') !== '' ) {

                            $morph_title.text( $( imageSet.small.element ).attr('alt') );
                        ...
                        if( !title_found && $this.parent('.gallery-icon').length ) {

                            if( $this.parent('.gallery-icon').next('.wp-caption-text').length ) {

                                $morph_title.text( $this.parent('.gallery-icon').next('.wp-caption-text').text() );
                        ...
                        if( !title_found && $this.find('h3').length && !$this.find('h3').hasClass('ut-image-gallery-empty-title') ) {

                            if( $this.find('h3').length ) {

                                $morph_title.text( $this.find('h3').text() );
            ...
            const UT_Morph_Box_Transition_Effect = UT_Morph_Box_Effect({

                activation: { type: "center" }, // center , corners, sides, sinX
                timing: {
            ...
            $(document).on('click', '.ut-morph-box-close', function(){
