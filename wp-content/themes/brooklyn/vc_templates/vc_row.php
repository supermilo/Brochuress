...
if( !empty( $atts['css'] ) && ut_vc_css_to_array( $atts['css'] ) ) {
	...
	if( !$parallax ) {
        ...
        if( $background_position ) {
            ...
            $current_vc_css['background-position'] = $background_position . ' !important;';

        } else {

            $current_vc_css['background-position'] = 'center center !important;';

        }

		$custom_css_style .= '#' . $el_id . ' .parallax-scroll-container {' . implode_with_key( $current_vc_css ) . '}';
