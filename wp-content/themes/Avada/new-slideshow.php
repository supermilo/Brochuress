		...
		<ul class="slides">
			...
			if ( 'grid' === $layout || 'masonry' === $layout ) {
				...
				if ( 'masonry' === $layout && has_post_thumbnail() ) {
					$element_orientation_class = Avada()->get_images_obj()->get_element_orientation_class( $thumbnail_id );
			...
			<?php while ( $i <= Avada()->settings->get( 'posts_slideshow_number' ) ) : ?>
				...
				<?php if ( $attachment_id ) : ?>
					...
					<?php $attachment_data = Avada()->get_images_obj()->get_attachment_data( $attachment_id ); ?>
						...
						<li>
							<div class="fusion-image-wrapper">
								<a href="<?php the_permalink(); ?>" aria-label="<?php the_title_attribute(); ?>">
									...
									$image_markup = Avada()->get_images_obj()->edit_grid_image_src( $image_markup, get_the_ID(), $attachment_id, $size );
									?>
