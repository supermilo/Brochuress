...
*/
class WarpHelperDom extends WarpHelper {
	...
	/*
		...
		Returns:
			Object
	*/
	function create($input, $mode = 'html') {
...
class WarpDOMElement extends DOMElement {
	...
	static function wrapNode(DOMNode $node, DOMNode $wrapper) {
	...
	static function deepestNode(DOMNode $node, $depth = 0, $current = null, &$deepest = null) {
	...
	static function attrClass($action, DOMNode $node, $class) {
...
class WarpDOMEntities {
	...
	static $regex = '/&([\w]+);|&#([\d]+);|&#(x[0-9a-fA-F]+);|(&)/m';
  
	static function replaceAllEntities($string) {
		return preg_replace_callback(self::$regex, 'WarpDOMEntities::doReplacement', $string);
	}
  
	static function doReplacement($matches) {
	...
	static function replaceEntity($entity) {
		return self::$entity_array[$entity];
	}
  
	static $entity_array = array(
...
class WarpCssSelectorParser {
	...
	static function convertSelector($selector, $prefix) {
